...
export GIT_ROOT
GIT_ROOT=$(cd "$BASEDIR" && cd "$(git rev-parse --show-cdup)" && pwd -P)

VERSIONS_FILE="${GIT_ROOT}/cmake/autogenerated_versions.txt"
...
function get_version {
    if [ -z "$VERSION_MAJOR" ] && [ -z "$VERSION_MINOR" ] && [ -z "$VERSION_PATCH" ]; then
        VERSION_REVISION=$(grep "SET(VERSION_REVISION" "$VERSIONS_FILE" | sed 's/^.*VERSION_REVISION \(.*\)$/\1/' | sed 's/[) ].*//')
        VERSION_MAJOR=$(grep "SET(VERSION_MAJOR" "$VERSIONS_FILE" | sed 's/^.*VERSION_MAJOR \(.*\)/\1/' | sed 's/[) ].*//')
        VERSION_MINOR=$(grep "SET(VERSION_MINOR" "$VERSIONS_FILE" | sed 's/^.*VERSION_MINOR \(.*\)/\1/' | sed 's/[) ].*//')
        VERSION_PATCH=$(grep "SET(VERSION_PATCH" "$VERSIONS_FILE" | sed 's/^.*VERSION_PATCH \(.*\)/\1/' | sed 's/[) ].*//')
    fi
...
function gen_dockerfiles {
    VERSION_STRING="$1"
    find docker -mindepth 2 -maxdepth 2 -name Dockerfile -print0 | \
      xargs -0 sed -i -r -e 's/ARG version=.+$/ARG version='"$VERSION_STRING"'/'
}
...
function make_rpm {
    ...
    {
      echo "Requires: clickhouse-common-static = $VERSION_FULL-2"
      echo "Requires: tzdata"
      echo "Requires: initscripts"
      echo "Obsoletes: clickhouse-server-common < $VERSION_FULL"
    } >> "${PACKAGE}-$VERSION_FULL-2.spec"
